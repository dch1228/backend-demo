	...
	return func(c *gin.Context) {
		defer func() {
			if err := recover(); err != nil {
				logger := log.Named("middleware.recovery")
				// try to get request logger
				if ctx, ok := c.Get("ctx"); ok {
					ctxTyped := ctx.(*model.ReqContext)
					logger = ctxTyped.Logger
				}
				...
				stack := stack(3)
				logger.Error("Request error", zap.Any("err", err), zap.String("stack", string(stack)))
